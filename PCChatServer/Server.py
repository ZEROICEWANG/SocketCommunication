...
LOCAL_IP='127.0.0.1'
class client:
    def __init__(self, connect, account,IP, username):
        ...
        with open(os.path.join(os.getcwd(),account,'info.txt')) as file:
            ...
            if  account == inf.split('\n')[1]:
                if password == inf.split('\n')[2]:
                    ...
                    cli=client(connect, account, connect.getpeername()[0], inf.split('\n')[0])
...
def IP_process(message,connect, account_clientDic_, logined_account_):
    ...
    if account in logined_account_:
        ...
        mess='IP|account='+account+'\'ip='+ip
    else:
        mess = 'IP|account=' + account + '\'ip=对方未登录'
...
def delete_process(message):
    ...
    sender=message.split('\'')[0].split('=')[1]
    with open(os.path.join(os.getcwd(), sender,'info.txt')) as file:
        inf=file.read()
        sender_name = inf.split('\n')[0]

    recv=message.split('\'')[1].split('=')[1]
    with open(os.path.join(os.getcwd(),recv,'info.txt')) as file:
        inf=file.read()
        recv_name = inf.split('\n')[0]
                    ...
                    for i in range(55535):
                        ...
                        if personalport in PORT_ARRAY:
                            ...
                            try:
                                ...
                                p = multiprocessing.Process(target=waiting_for_connection,
                                                            args=(personalport, connectlist))
                                p.start()
